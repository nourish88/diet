...
export default function DietitianMessagesScreen() {
  ...
  const { id: clientId, dietId } = useLocalSearchParams();
  ...
  useEffect(() => {
    if (!dietId) {
      Alert.alert("Hata", "Diyet bilgisi bulunamadı");
      router.back();
      return;
    }
    loadMessages();
        ...
        <View style={styles.header}>
          ...
          <View style={styles.headerCenter}>
            <Text style={styles.headerTitle}>
              {clientInfo ? `${clientInfo.name} ${clientInfo.surname}` : "Danışan"}
            </Text>
                  ...
                  >
                    {!isMe && (
                      <Text style={styles.senderName}>
                        {message.user.role === "client" ? "Danışan" : "Ben"}
                      </Text>
                    )}

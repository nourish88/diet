  ...
  async (request: NextRequest, auth: AuthResult) => {
    try {
      ...
      const diets = await prisma.diet.findMany({
        where: {
          dietitianId: auth.user!.id, // SECURITY: Only get own diets
        },
        take: 30,
        orderBy: {
          createdAt: "desc",
    ...
    for (const [normalizedMealKey, meals] of Object.entries(mealGroups)) {
      ...
      // Store the normalized meal key with each pattern for later use
      patterns.forEach(pattern => {
        (pattern as any).normalizedMealKey = normalizedMealKey;
      });
    ...
    for (const pattern of detectedPatterns) {
      ...
      // Get meal type from the normalized meal key (not from pattern name)
      const normalizedMealKey = (pattern as any).normalizedMealKey || "diğer";
      ...
      if (existing) {
        // Update score and mealType if needed
        await prisma.mealPreset.update({
          where: { id: existing.id },
          data: {
            patternScore: pattern.score,
            mealType: mealType, // Update mealType in case it was wrong before
          },
        });
      } else {
        // Create new preset
...
// Helper: Get meal type category from normalized meal group key
function getMealTypeCategory(normalizedMealKey: string): string {
  ...
  // For "diğer", try to determine from the original name if available
  return "diger";
}

// Helper: Get meal type category from meal name (for fallback)
function getMealTypeCategoryFromName(name: string): string {
  const lower = name.toLowerCase();
  if (lower.includes("uyanınca") || lower.includes("uyaninca")) return "uyaninca";
